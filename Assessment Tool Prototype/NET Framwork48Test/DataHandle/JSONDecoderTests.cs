...
namespace NET_Framwork48.DataHandle.Tests
{
    ...
    public class JSONDecoderTests
    {
        ...
        {
            string path = @"jsonDataSample.json";
            ...
            Assert.AreEqual(2, root.repositoryInfo.Count);
            Assert.AreEqual("repositoryURL1", root.repositoryInfo[0].repositoryURL);
            ...
            Assert.AreEqual(2, root.microservices.Count);
            Assert.AreEqual("microservice name1", root.microservices[0].serviceName);
            Assert.AreEqual(2, root.microservices[0].interfaces.Count);
            ...
            Assert.AreEqual(2, root.microservices[0].call.Count);
            ...
            Assert.AreEqual(2, root.messages.Count);
            Assert.AreEqual("messageName1", root.messages[0].messageName);
            Assert.AreEqual(1122, root.messages[0].parameterNumber);
